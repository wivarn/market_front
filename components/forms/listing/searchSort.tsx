...
export function SearchSort(): JSX.Element {
  ...
  let initialSelected = searchSortOptions[0];
  const params = new URLSearchParams(router.asPath.split("?")[1]);
  initialSelected =
    searchSortOptions.find((option) => option.id == params.get("sort")) ||
    searchSortOptions[0];
        ...
        <div>
          ...
          >
            <Listbox.Options className="absolute right-0 z-10 px-2 py-2 my-2 text-sm bg-white border rounded-md shadow-lg md:left-0 md:w-full md:text-base border-accent">
              {searchSortOptions.map((option, optionId) => (
                <Listbox.Option
                  key={optionId}
                  className={({ active }) =>
                    `${
...
export function ListSort(): JSX.Element {
  const router = useRouter();

  let initialSelected = listSortOptions[0];
  const params = new URLSearchParams(router.asPath.split("?")[1]);
  initialSelected =
    listSortOptions.find((option) => option.id == params.get("sort")) ||
    listSortOptions[0];
    ...
    <div className="relative md:w-72">
      <Listbox value={selected} onChange={setSelected}>
        <div>
          ...
          >
            <Listbox.Options className="absolute right-0 z-10 px-2 py-2 my-2 text-sm bg-white border rounded-md shadow-lg md:left-0 md:w-full md:text-base border-accent">
              {listSortOptions.map((option, optionId) => (
                <Listbox.Option
                  key={optionId}
                  className={({ active }) =>
                    `${
